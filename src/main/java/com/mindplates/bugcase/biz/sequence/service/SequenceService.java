...
public class SequenceService {
    ...
    public DirectedGraph selectProjectSequenceGraph(long projectId) {
        ...
        sequences.forEach((sequence) -> {
            sequence.getNodes().forEach((node) -> {
                graph.addNode(node.getTestcase().getId());
            });

            sequence.getEdges().forEach((edge) -> {
                graph.addEdge(edge.getSource().getTestcase().getId(), edge.getTarget().getTestcase().getId());
            });
        });
